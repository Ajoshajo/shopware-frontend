...
import { ClientOnly } from "@shopware/cms-base-layer/helpers/clientOnly";
import { isSpatial } from "@shopware/cms-base-layer/helpers/media/isSpatial";
...
const SwMedia3D = computed(() => {
  if (isSpatial(props.content.data.media)) {
    return defineAsyncComponent(() => import("@shopware/cms-base-layer/components/SwMedia3D.vue"));
  }
  return "";
});
</script>
<template>
  ...
  <component
    ...
    v-if="imageAttrs.src"
    class="cms-element-image relative h-full w-full p-2"
    :class="{
