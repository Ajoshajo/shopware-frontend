...
<template>
  ...
  <nav
    class="hidden lg:flex justify-start space-x-4 items-center text-center container mx-auto px-4 md:px-6"
  ...
  >
    <div
      ...
      @focusout="currentMenuPosition = undefined"
    >
      <NuxtLink
        role="menuitem"
        :target="
        ...
        class="text-base font-medium hover:text-secondary-900 p-2 flex items-center gap-1"
        @click="
          updateActiveClass(navigationElement.id, navigationElement.parentId)
        "
      >
        {{ getTranslatedProperty(navigationElement, "name") }}
        <img v-if="navigationElement.children.length > 0" src="~/assets/icons/down.svg" class="mt-0.5 w-3 h-3">
      </NuxtLink>
      ...
      <client-only>
        <div
          ...
          class="absolute z-10 transform px-2 w-screen max-w-sm sm:px-0 lg:ml-0"
          :class="{
        ...
        >
          ...
          >
            ...
            <div
              v-if="navigationElement.description"
